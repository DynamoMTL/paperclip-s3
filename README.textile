...
You can configure the path for all attachments using the following:

<pre>
  Paperclip::S3.config do |config|
    config.path = "assets/:class/:attachment/:id/:style/:basename.:extension"
  end
</pre>
